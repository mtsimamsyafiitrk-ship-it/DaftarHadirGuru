...
import { getFirestore, doc, getDoc, setDoc, deleteDoc, updateDoc, collection, getDocs } from "https://www.gstatic.com/firebasejs/12.10.0/firebase-firestore.js";
...
const DS=["Ahd","Sen","Sel","Rab","Kam","Jum","Sab"]; // lookup by JS getDay()
const DF=["Ahad","Senin","Selasa","Rabu","Kamis","Jumat","Sabtu"];
// Kalender 6 kolom, awal pekan Sabtu: col0=Sab(6),col1=Ahd(0),col2=Sen(1),col3=Sel(2),col4=Rab(3),col5=Kam(4)
const CAL_COLS=["Sab","Ahd","Sen","Sel","Rab","Kam"];
const CAL_JS=[6,0,1,2,3,4]; // JS getDay() per kolom
const TODAY=new Date();
const ADMIN_DEFAULT_PW="Madras0h!";
// ── CUSTOM USER ORDER ──
...
const CUSTOM_ORDER=[
  ...
  ["MUHAMMAD IHSAN"],                                // 19 - hati2 urutan setelah IHSAN FAUZY
  ["MUHAMMAD SYUKUR"],                               // 20
...
function getOrderIndex(name){
  ...
  for(let i=0;i<CUSTOM_ORDER.length;i++){
    for(const kw of CUSTOM_ORDER[i]){
      if(upper.includes(kw)||kw.split(' ').every(w=>w.length>2&&upper.includes(w)))return i;
    }
  }
  return 9999;
}
...
// roles can be array or legacy string
function getRoles(u){
...
function rolesText(u){return getRoles(u).join(', ');}
...
// Enkripsi ringan untuk simpan password plain (hanya untuk admin)
const PW_KEY='AlImamAsySyafi';
function encodePw(pw){
  let r='';
  for(let i=0;i<pw.length;i++)r+=String.fromCharCode(pw.charCodeAt(i)^PW_KEY.charCodeAt(i%PW_KEY.length));
  return btoa(unescape(encodeURIComponent(r)));
}
function decodePw(enc){
  try{
    const r=decodeURIComponent(escape(atob(enc)));
    let out='';
    for(let i=0;i<r.length;i++)out+=String.fromCharCode(r.charCodeAt(i)^PW_KEY.charCodeAt(i%PW_KEY.length));
    return out;
  }catch(e){return '••••••';}
}
...
// ── HARI LIBUR: Firestore helpers ──
// Menyimpan semua tanggal libur yang pernah diterapkan admin
let holidayDatesSet = new Set(); // Set of "YYYY-MM-DD"

async function getHolidayDates(){
  const d = await getDoc(doc(fs,'config','holidays'));
  return d.exists() ? (d.data().dates || []) : [];
}
async function saveHolidayDates(newDates){
  // Merge dengan yang sudah ada (akumulatif)
...
// ── RINCIAN HARI LIBUR ──
let _rincianDates = []; // cache daftar hari libur untuk render

async function refreshRincianLibur(){
  ...
  try{
    ...
    document.getElementById('hr-loading').innerHTML = `<div style="color:var(--rose2);font-weight:700">❌ Gagal memuat: ${e.message}</div>`;
  }
}
window.refreshRincianLibur = refreshRincianLibur;

function renderRincianLibur(){
  ...
  const filterDesc = selYear==='-1'?'semua tahun':(selMonth==='-1'?`tahun ${selYear}`:`${MONTHS[parseInt(selMonth)]} ${selYear}`);
...
async function hapusSatuHariLibur(dateStr){
...
async function hapusSemuaHariLibur(){
...
const fd=(y,m)=>{
  ...
  if(js===5){ // Jika tgl 1 = Jumat, skip ke Sabtu berikutnya = offset 0 baris baru
    return 0; // tgl 1 Jumat tidak ditampilkan, tgl 2 Sabtu masuk kolom 0
  }
  const c=CAL_JS.indexOf(js);
  return c>=0?c:0;
};
const wom=(d,f)=>Math.ceil((d+f)/7);
const wim=(y,m)=>Math.ceil((dim(y,m)+fd(y,m))/7);
const dk=(y,m,d)=>`${y}-${String(m+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
const emptyDay=()=>SESSIONS.reduce((a,s)=>({...a,[s.key]:false}),{});
const cntDay=dd=>SESSIONS.filter(s=>dd[s.key]).length;
const scDay=dd=>cntDay(dd)*2;
...
async function doLogin(){
  ...
  try{
    if(loginRole==='admin'){
      ...
      showScreen('user-att');
      switchTab('monthly'); // render kalender SETELAH holidayDatesSet terisi
...
function printRekap(){
  ...
  <div style="display:flex;align-items:flex-start;gap:16px;border-bottom:4px solid #1a5c2a;padding-bottom:10px;margin-bottom:14px">
    <img src="data:image/png;base64," style="width:70px;height:70px;border-radius:50%;object-fit:cover;flex-shrink:0"/>
    <div style="flex:1">
  ...
  try{
    ...
    for(const user of users){
      ...
      for(let d=1; d<=dim2; d++){
        ...
        const dateKey = dk(y,m,d);
        const isLibur = holidayDatesSet.has(dateKey);
...
window.exportUsersPdf=()=>{
  ...
  <div class="kop">
    <div style="display:flex;align-items:center;justify-content:center;gap:14px;margin-bottom:6px">
      <img src="data:image/png;base64," style="width:64px;height:64px;border-radius:50%;object-fit:cover"/>
      <div>
...
window.__tog=async(uid,y,m,d,sk)=>{
  ...
  if(holidayDatesSet.has(dateKey)){ showHolidayNotif(); return; }
